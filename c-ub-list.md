...
| 3 | 预处理符号连接产生了通用字符名称 | 5.1.1.2 |
| 4 | 宿主环境（hosted environment）中的程序没有按要求定义 main 函数 | 5.1.2.2.1 |
| 5 | 具有数据竞争（data race）的程序 | 5.1.2.4 |
| 6 | 除标识符、字符或字符串常量、头文件名称、不会生成符号的预处理符号之外，存在非基本字符集中的字符 | 5.2.1 |
| 7 | 标识符、注释、字符或字符串常量、头文件名称包含非法字符 | 5.2.1.2 |
| 8 | 在一个翻译单元中同一个标识符既具有内部连接性又具有外部连接性 | 6.2.2 |
| 9 | 在对象生命周期之外访问对象 | 6.2.4 |
| 10 | 指针指向生命周期已结束的对象，并被使用 | 6.2.4 |
| 11 | 使用未初始化的具有自动存储期的对象 | 6.2.4<br/>6.7.9<br/>6.8 |
...
| 21 | 通过 lvalue 访问未初始化对象的值 | 6.3.2.1 |
...
| 58 | 需要常量表达式时，可以用 \[ \]、.、\->、&、\* 等运算符生成常量地址，但不可通过这些运算符访问对象的值 | 6.6 |
...
| 66 | 函数类型被 const、volatile、restrict、\_Atomic 等 type qualifiers 限定（通过 typedef） | 6.7.3 |
| 67 | 应该兼容的类型不具备相同的类型限定符（const、volatile） | 6.7.3 |
...
| 74 | 应该兼容的指针类型不具备相同的类型限定符（const、volatile），或指向不同的类型 | 6.7.6.1 |
| 75 | 数组声明中的长度表达式不是常量表达式或结果不是正整数 | 6.7.6.2 |
...
| 89 | 试探性定义（tentative definition）具有内部链接和不完整类型的对象 | 6.9.2 |
| 90 | 在 \#if、 \#elif 的条件中，由宏展开产生了 defined 表达式，或 defined 关键字格式不正确 | 6.10.1 |
| 91 | \#include 指令不符合标准格式 | 6.10.2 |
| 92 | \#include 指令中头文件名称不是以字母开头 | 6.10.2 |
| 93 | 宏的实参列表中出现预处理指令 | 6.10.3 |
| 94 | 预处理运算符 \# 的结果不是有效的字符串文本 | 6.10.3.2 |
...
| 105 | 程序不通过标准头文件声明库函数，但声明不具备外部连接性 | 7.1.2 |
| 106 | 程序定义或声明了不被许可的保留标识符名称 | 7.1.3 |
| 107 | 程序去除了以下划线和大字母以及两个下划线开头的宏定义 | 7.1.3 |
| 108 | 为有可变参数列表的库函数提供具有无效值或无效类型的参数 | 7.1.4 |
| 109 | 对于传递给库函数的数组，不能保证所有地址计算和对象访问是有效的 | 7.1.4 |
| 110 | assert 宏被抑制 | 7.2 |
| 111 | assert 宏的参数不具备标量类型（scalar type） | 7.2 |
| 112 | 在不适当的场合进行与 CX\_LIMITED\_RANGE、FENV\_ACCESS、FP\_CONTRACT 相关的预处理设置 | 7.3.4<br/>7.6.1<br/>7.12.2 |
...
| 117 | fesetexceptflag 没有正确地有与 fegetexceptflag 配合使用 | 7.6.2.4 |
| 118 | fesetenv 没有正确地有与 fegetenv 配合使用，feupdateenv 没有正确地有与 feholdexcept 配合使用 | 7.6.4.3<br/>7.6.4.4 |
| 119 | 整数运算或转换函数的结果不能被有效表示 | 7.8.2.1<br/>7.8.2.2<br/>7.8.2.3<br/>7.8.2.4<br/>7.22.6.1<br/>7.22.6.2<br/>7.22.1 |
...
| 123 | 为浮点数分类或比较相关的宏提供非真浮点类型的参数 | 7.12.3<br/>7.12.14 |
...
| 131 | 处理 abort 或 raise 函产生的信号时，在信号处理函数中调用 raise 函数 | 7.14.1.1 |
| 132 | 在信号处理函数中引用非 lock\-free 具有静态或线程存储期的对象，或使用 abort、\_Exit、quick\_exit、signal 之外的库函数 | 7.14.1.1 |
| 133 | 在信号处理函数中调用 signal 函数，当其返回 SIG\_ERR 时访问 errno 的值 | 7.14.1.1 |
| 134 | 异步信号处理函数产生信号 | 7.14.1.1 |
| 135 | 在多线程环境中使用 signal 函数 | 7.14.1.1 |
| 136 | 没有正确地通过 va\_list 访问可变参数，或在使用 va\_start 之前访问可变参数 | 7.16<br/>7.16.1.1<br/>7.16.1.4 |
...
| 139 | va\_start 或 va\_copy 没有正确地与 va\_end 配合使用 | 7.16.1<br/>7.16.1.2<br/>7.16.1.3<br/>7.16.1.4 |
| 140 | va\_arg 的类型参数后面加 \* 号不能表示指针 | 7.16.1.1 |
| 141 | 没有实际的下一个参数时使用 va\_arg 宏，或者下一个参数的类型与 va\_arg 声明的类型不兼容 | 7.16.1.1 |
| 142 | 用 va\_copy 或 va\_start 重复初始化 va\_list，已被 va\_end 关闭的除外 | 7.16.1.2<br/>7.16.1.4 |
| 143 | parmN（省略号左侧的参数）具有寄存器存储期，或函数、数组等类型 | 7.16.1.4 |
| 144 | offset 的第二个参数不是有效的 . 运算符右子表达式，或者是位域 | 7.19 |
| 145 | integer\-constant 宏（如 INT32\_C、UINT64\_C 等）的参数不是合法的十进制、八进制或十六进制常量，或者参数的值超过相应类型的范围 | 7.20.4 |
| 146 | 对面向宽字符的流使用面向字节的输出输出函数，或对面向字节的流使用面向宽字符的输出输出函数 | 7.21.2 |
| 147 | 写入的宽字符覆盖了多节字字符的一部分，而后面的内容被使用 | 7.21.2 |
| 148 | 在文件对象被关闭后仍被使用 | 7.21.3 |
...
| 152 | 对提供给 setvbuf 的数组，访问其内容 | 7.21.5.6 |
| 153 | 提供给格式化函数的参数数量不足，或者参数的类型不符合要求 | 7.21.6.1<br/>7.21.6.2<br/>7.29.2.1<br/>7.29.2.2 |
| 154 | 对格式化函数提供的格式化参数不是符合要求的多字符序列 | 7.21.6.1<br/>7.21.6.2<br/>7.27.3.5<br/>7.29.2.1<br/>7.29.2.2<br/>7.29.5.1 |
| 155 | 对无精度要求的格式化占位符设定精度 | 7.21.6.1<br/>7.29.2.1 |
| 156 | 设置了可变宽度或精度的格式化占位符，但并未提供与宽度或精度对应的参数 | 7.21.6.1<br/>7.29.2.1 |
| 157 | 用 \# 修饰 o、x、X、a、A、e、E、f、F、g、G 之外的格式化占位符，用 0 修饰 d、i、o、u、x、X、a、A、e、E、f、F、g、G 之外的格式化占位符 | 7.21.6.1<br/>7.29.2.1 |
| 158 | 用 length modifier 修饰 d、i、o、u、x、X、a、A、e、E、f、F、g、G 之外的格式化占位符 | 7.21.6.1<br/>7.21.6.2<br/>7.29.2.1<br/>7.29.2.2 |
| 159 | 格式化占位符 s 对应的字符串参数不是以空字符结尾，除非对占位符 s 声明不需要空字符结尾的精度 | 7.21.6.1<br/>7.29.2.1 |
| 160 | 对格式化占位符 n 声明任何宽度、精度、标志位、赋值抑制字符 | 7.21.6.1<br/>7.21.6.2<br/>7.29.2.1<br/>7.29.2.2 |
| 161 | 格式化占位符 % 不符合 %% 这种格式 | 7.21.6.1<br/>7.21.6.2<br/>7.29.2.1<br/>7.29.2.2 |
| 162 | 使用无效的格式化占位符 | 7.21.6.1<br/>7.21.6.2<br/>7.27.3.5<br/>7.29.2.1<br/>7.29.2.2<br/>7.29.5.1 |
| 163 | 格式化输出函数参数过多，数量超过 INT\_MAX | 7.21.6.1<br/>7.29.2.1 |
| 164 | 格式化输入函数参数过多，数量超过 INT\_MAX | 7.21.6.2<br/>7.29.2.2 |
| 165 | 格式化占位符与对应的参数类型不匹配 | 7.21.6.2<br/>7.29.2.2 |
| 166 | 格式化输入函数的 c、s、\[ 占位符对应的参数没有足够的空间容纳输入的数据 | 7.21.6.2<br/>7.29.2.2 |
| 167 | 由 l 修饰的格式化输入占位符 c、s、\[ 对应的输入不是符合要求的多字符序列 | 7.21.6.2<br/>7.29.2.2 |
| 168 | 格式化输入函数中 %p 对应的参数不是同一个进程中的指针 | 7.21.6.2<br/>7.29.2.2 |
| 169 | 未正确初始化的 va\_last 作为 vfprintf、vfwprintf、vscanf、vswscanf 等函数的参数，或者在函数返回后继续使用该参数 | 7.21.6.8<br/>7.21.6.9<br/>7.21.6.10<br/>7.21.6.11<br/>7.21.6.12<br/>7.21.6.13<br/>7.21.6.14<br/>7.29.2.5<br/>7.29.2.6<br/>7.29.2.7<br/>7.29.2.8<br/>7.29.2.9<br/>7.29.2.10 |
| 170 | 在 fgets 或 fgetws 失败后继续使用传入的数组 | 7.21.7.2<br/>7.29.3.2 |
| 171 | 在调用 ungetc 函数后使用二进制流的 file position indicator，而在调用之前其值为 0 | 7.21.7.10 |
| 172 | 在 fread 或 fwrite 失败后使用 file position indicator | 7.21.8.1<br/>7.21.8.2 |
...
| 178 | aligned\_alloc 要求的对齐方式当前实现不支持，或申请的长度不是其整数倍 | 7.22.3.1 |
| 179 | free 或 realloc 的参数不是由匹配的分配函数返回，或是已被释放的地址 | 7.22.3.3<br/>7.22.3.5 |
| 180 | 使用由 malloc 分配的且未初始化的数据 | 7.22.3.4 |
| 181 | 使用由 realloc 分配的超出原对象大小的且未初始化的数据 | 7.22.3.5 |
| 182 | 调用 exit 或 quick\_exit 超过一次，或调用了这两个函数 | 7.22.4.4<br/>7.22.4.7 |
| 183 | 调用由 atexit 或 at\_quick\_exit 注册的函数时，调用 longjmp 结束了被注册函数的执行 | 7.22.4.4<br/>7.22.4.7 |
| 184 | 程序修改了由 getenv 或 strerror 引入的字符串 | 7.22.4.6<br/>7.24.6.2 |
| 185 | 当 quick\_exit 函数执行时产生了信号 | 7.22.4.7 |
...
| 191 | 字符串或宽字符串库函数越界访问相关对象 | 7.24.1<br/>7.29.4 |
| 192 | 无效指针或无效长度作为字符串或宽字符串库函数的参数 | 7.24.1<br/>7.29.4 |
| 193 | strxfrm、strftime、wcsxfrm、wcsftime 的目标数组过小，无法存储以空字符结尾的完整结果 | 7.24.4.5<br/>7.27.3.5<br/>7.29.4.4.4<br/>7.29.5.1 |
| 194 | 第一次调用 strtok 或 wcstok 时，第一个参数是空指针 | 7.24.5.8<br/>7.29.4.5.7 |
