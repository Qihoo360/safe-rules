...
用户可在 ID\_include 一节的末尾设置不合规头文件的名称。  
...
```
例中 B 和 C 对 foo 函数的重写是无效的，成员 a 的值总为 0，这会使维护者产生相当大的误解。
<br/>
<br/>
...
```
例中 B 对 clear 函数的重写是无效的，相关内存也没有被释放，这会使维护者产生相当大的误解。
<br/>
<br/>
...
函数返回 const 对象不利于移动构造或移动赋值等优化机制，也可能本意是返回引用，但遗漏了引用符号。  
