...
属性为noreturn的函数中出现return语句说明函数还是可以正常返回的，这种矛盾会对调用者造成很大困扰，而且会导致标准未定义的问题。  
...
```
例中B是对A的某种改造，如果再用A的方法去处理B的对象，显然是有问题的。
<br/>
<br/>
...
需要注意的是，指向虚基类的指针只能通过dynamic\_cast转换为指向派生类的指针，否则导致标准未定义的错误：
```
struct A { .... };
struct B: virtual A { .... };
struct C: virtual A { .... };
