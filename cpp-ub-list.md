...
# C++ 未定义行为列表 ![Version](https://img.shields.io/badge/version-1.1.1-brightgreen)

未定义的行为（Undefined Behavior），指程序不可预测的执行效果，一般由错误的代码实现引起，出于效率、兼容性等多方面原因，语言标准不便于定义错误程序的明确行为，故将错误程序的行为统称为“未定义”的行为。未定义的行为可以是崩溃，也可以是非预期的任意表现，代码质量管理的一个重要目标就是消除会导致未定义行为的代码。

C\+\+ 标准声明了导致未定义行为的编码方式，本文梳理了 ISO/IEC 14882:2003 和 ISO/IEC 14882:2011 前 18 章的相关内容，后 12 章的主题为标准库实现，相关内容在前 18 章中已有体现。
 
<br/>

|   ID   | Summary |  ISO\-Reference  |
| :----: |  :----  |     :----       |
...
| 11 | [全局、静态或 thread\_local 对象的析构函数调用 std::exit 函数](#_11) | [`11-3.6.1(4)`](#_11) |
...
| 18 | [对象生命周期已结束，但未调用析构函数，且析构函数有副作用](#_18) | [`11-3.8(4)`](#_18) |
| 19 | [在分配内存空间后对象生命周期开始前，或在对象生命周期结束后回收内存空间前，通过指针访问对象](#_19) | [`11-3.8(5)`](#_19) |
| 20 | [在分配内存空间后对象生命周期开始前，或在对象生命周期结束后回收内存空间前，通过 glvalue 访问对象](#_20) | [`11-3.8(6)`](#_20) |
| 21 | [具有 non\-trivial 析构函数的对象，其空间被不兼容类型的对象占据](#_21) | [`11-3.8(8)`](#_21) |
...
| 25 | [浮点类型转换产生的结果无法在提供的空间中表示](#_25) | [`11-4.8(1)`](#_25) |
| 26 | [浮点类型转为整数类型时，整数类型无法存储浮点类型的整数部分](#_26) | [`11-4.9(1)`](#_26) |
| 27 | [整数类型转为浮点类型时，浮点类型无法存储整数的值](#_27) | [`11-4.9(2)`](#_27) |
| 28 | [在序列点之间多次读写同一对象](#_28) | [`03-5(4)`](#_28) |
| 29 | [表达式的结果未在数学上定义或超过其类型的范围](#_29) | [`11-5(4)`](#_29) |
...
| 37 | [对不完整类型的对象取地址，而该对象的完整类型重载了 operator &](#_37) | [`11-5.3.1(5)`](#_37) |
...
| 41 | [用 delete 释放不完整类型的对象，而在对象完整类型声明中有 non\-trivial 析构函数](#_41) | [`11-5.3.5(5)`](#_41) |
...
| 50 | [函数有返回值，但没有返回明确的值](#_50) | [`11-6.6.3(2)`](#_50) |
...
| 54 | [在属性为 noreturn 的函数中使用 return 语句或隐式返回](#_54) | [`11-7.6.3(2)`](#_54) |
...
| 66 | [在面向构造或析构函数体的 catch handler 中访问非静态成员](#_66) | [`11-15.2(10)`](#_66) |
...
| 71 | [预处理运算符 \# 的结果不是有效的字符串文本](#_71) | [`11-16.3.2(2)`](#_71) |
...
| 77 | [程序实现了应由标准库提供的函数](#_77) | [`11-17.3.22`](#_77) |
...
| 87 | [可变参数列表 ... 左侧的形式参数为函数、数组或引用](#_87) | [`11-18.10(3)`](#_87) |
...
### <span id="_11">11. 全局、静态或 thread\_local 对象的析构函数调用 std::exit 函数</span>
<br/>

调用 exit 函数后，具有静态或线程存储期的对象开始析构，而这种对象的析构函数再调用 exit 函数会导致未定义的行为。  
...
### <span id="_18">18. 对象生命周期已结束，但未调用析构函数，且析构函数有副作用</span>
<br/>
...
T* pT = new (p) T;
U* pU = new (p) U;     // Undefined behavior if ‘U::~U’ has side effects
```
...
### <span id="_19">19. 在分配内存空间后对象生命周期开始前，或在对象生命周期结束后回收内存空间前，通过指针访问对象</span>
<br/>

在分配内存空间后对象生命周期开始前，或在对象生命周期结束后回收内存空间前，如果通过指针进行如下操作：  
 - 通过指针访问非静态成员函数  
 - 将指针转为基类指针  
...
### <span id="_20">20. 在分配内存空间后对象生命周期开始前，或在对象生命周期结束后回收内存空间前，通过 glvalue 访问对象</span>
<br/>

在分配内存空间后对象生命周期开始前，或在对象生命周期结束后回收内存空间前，如果通过 glvalue 进行如下操作：  
 - 对 glvalue 进行 lvalue\-to\-rvalue 的转换  
 - 通过 glvalue 访问非静态成员函数  
 - 将 glvalue 转为基类引用  
...
### <span id="_21">21. 具有 non\-trivial 析构函数的对象，其空间被不兼容类型的对象占据</span>
<br/>
...
### <span id="_25">25. 浮点类型转换产生的结果无法在提供的空间中表示</span>
<br/>
...
### <span id="_28">28. 在序列点之间多次读写同一对象</span>
<br/>
...
### <span id="_29">29. 表达式的结果未在数学上定义或超过其类型的范围</span>
<br/>
...
### <span id="_37">37. 对不完整类型的对象取地址，而该对象的完整类型重载了 operator &</span>
<br/>
...
### <span id="_41">41. 用 delete 释放不完整类型的对象，而在对象完整类型声明中有 non\-trivial 析构函数</span>
<br/>
...
### <span id="_50">50. 函数有返回值，但没有返回明确的值</span>
<br/>
...
### <span id="_54">54. 在属性为 noreturn 的函数中使用 return 语句或隐式返回</span>
<br/>
...
对具有 non\-trivial 构造函数的对象，在构造函数执行之前引用其非静态成员或基类对象，对具有 non\-trivial 析构函数的对象，在析构函数执行之后引用其非静态成员或基类对象，会导致未定义的行为。  
...
### <span id="_66">66. 在面向构造或析构函数体的 catch handler 中访问非静态成员</span>
<br/>
...
### <span id="_71">71. 预处理运算符 \# 的结果不是有效的字符串文本</span>
<br/>
...
### <span id="_77">77. 程序实现了应由标准库提供的函数</span>
<br/>
...
### <span id="_87">87. 可变参数列表 ... 左侧的形式参数为函数、数组或引用</span>
<br/>
