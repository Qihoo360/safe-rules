...
| 44 | [/ 或 % 运算符的第二个操作数的值为 0](#_44) | [`11-5.6(4)`](#_44) |
...
```
例中变量 u 的值一定是 0，而 s 的值是标准未定义的，往往由编译器的具体实现决定。  
...
```
例中 foo 函数没有返回值，但将其强转为有返回值的函数并调用，会导致未定义的行为。
<br/>
<br/>
...
### <span id="_44">44. / 或 % 运算符的第二个操作数的值为 0</span>
<br/>
...
```
如果实例化 A，需要实现化 A<T\*>，如果实例化 A<T\*>，需要实现化 A<T\*\*>，依次类推，形成了无限递归。
<br/>
<br/>
